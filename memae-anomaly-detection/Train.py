...
if args.dataset_type == "Avenue_rain":
    for single_dir in train_folder:
        if not os.path.exists(single_dir):
            rain_type = single_dir.strip().split('frames/')[1].strip().split("_")[0]
            print("creating training images with augmentation %s and rain level 0.70" % rain_type)
            aug_data.save_avenue_rain_or_bright(args.dataset_path, rain_type, True, "training", bright_space=0.7)
